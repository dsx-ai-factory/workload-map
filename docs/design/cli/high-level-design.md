...
**File mode.** `karta describe -f jobset.yaml` builds the same view from a manifest alone, resolving the definition by the manifest's GVK, with no cluster and no pods. A manifest written at another served version of a covered kind still resolves, through the definition for that group and kind. The file holds one workload: a stream of several documents is rejected rather than read up to its first. The structure and the desired scale are real; everything live is left empty and the output says so. Useful for checking a desired topology before submitting it.
